...
class WorkFeishuPush implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $client = new Client();
        $apiUrl = dujiaoka_config_get('feishubot_key');
        $params = [
            "msgtype"=>"markdown",
            "markdown"=>[
                "content"=>__('dujiaoka.prompt.new_order_push').'(<font color="warning">'.$this->order->actual_price."</font>元)\n"
                    ...
                    .'>'.__('order.fields.search_pwd') .': <font color="comment">'.$this->order->search_pwd."</font>\n"
